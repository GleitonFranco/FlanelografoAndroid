package com.example.flanelografoandroid;
...
import com.example.flanelografoandroid.util.SystemUiHider;
...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this, 
				R.array.lista_cursos, android.R.layout.simple_spinner_item);
		adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		spnCurso.setAdapter(adapter);
		
		btnAtualiza.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				edtSala.setText(solicitaSala(spnCurso.getSelectedItemPosition()));
			}
		});
	}
	...
	public CharSequence solicitaSala(Integer indice) {
		CharSequence s = "";
		try {
			SoapObject envio = new SoapObject(NAMESPACE,OPERATION);
			SoapSerializationEnvelope envelope = new SoapSerializationEnvelope(SoapEnvelope.VER11);
			envio.addProperty("curso",indice.toString());
			...
			s=s+"2";
			s=s+resultado.getProperty(0).toString();
		} catch(Exception e) {
			s=s+" ---"+e.getMessage();
//			return e.getMessage();
